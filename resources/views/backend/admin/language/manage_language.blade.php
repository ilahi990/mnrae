        ...
        <div class="eSection-wrap pb-2">
        	<ul class="nav nav-tabs eNav-Tabs-custom"id="myTab"role="tablist" >
        		...
        		<li class="nav-item" role="presentation">
                  <button  class="nav-link active"  id="upcoming-tab" data-bs-toggle="tab"
                    data-bs-target="#editphrase" type="button"  role="tab"  aria-controls="editphrase" aria-selected="false"> {{ get_phrase('Edit phrase ') }}
                    <span></span>
                  </button>
                </li>
                <?php endif;?>
                ...
                <li class="nav-item" role="presentation">
                  <button class="nav-link {{ empty($edit_profile) ? 'active':'' }}" id="upcoming-tab"
                    data-bs-toggle="tab" data-bs-target="#languagelist" type="button"
                    role="tab"  aria-controls="languagelist" aria-selected="false">
                ...
                <li class="nav-item" role="presentation">
                    <button  class="nav-link" id="archive-tab"  data-bs-toggle="tab" data-bs-target="#addlanguage" type="button" role="tab" aria-controls="addlanguage" aria-selected="false">
            ...
            <div class="tab-content eNav-Tabs-content" id="nav-tabContent">
            		...
            		<div class="table-responsive-sm">
						<table class="table eTable">
							...
							<tbody>
								...
								<tr>
									<td>{{ ucwords($language->identifier) }}</td>
									<td>
										<a href="{{ route('admin.language.manage', ['language' => $language->identifier]) }}">
											<button type="button" class="btn-form btn-sm">{{ get_phrase('Edit phrase') }}</button>
										</a>
										@if($language->identifier != 'english')
										<button type="button" class="btn-form btn-danger btn-sm" onclick="confirmModal('{{ route('admin.language.delete', ['identifier' => $language->identifier]) }}', 'undefined')">{{ get_phrase('Delete language') }}</button>
										@else
            	...
            	<div class="tab-pane fade pt-3  " id="addlanguage" role="tabpanel" aria-labelledby="addlanguage-tab">
            		<div class="row">
						<div class="col-xl-5">
							<form class="" action="{{ route('admin.language.add') }}" method="post">
								@csrf
								<div class="fpb-7">
									<label for="language" class="eForm-label">{{ get_phrase('Add new language') }}</label>
									<input type="text" id="language" name="language" class="form-control eForm-control" placeholder="{{ get_phrase('No special character or space is allowed') }}">
									<p class="eCard-text">
...
<script type="text/javascript">
	...
	function updatePhrase(phrase, language, phrase_id) {
		...
		let url = "{{ route('admin.language.update_phrase', ['language' => ":language"]) }}";
