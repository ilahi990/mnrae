            ...
            @foreach($message_threads as $row)
              ...
              <li class="nav-item" role="presentation">
                <div
                  ...
                  aria-selected="true"
                >
                  <a href="{{ route('customerMesssage', ['param1' => 'message_read', 'param2' => $row->message_thread_code]) }}">
                    <div
          ...
          <div class="message-right">
            ...
            >
              ...
              >
                ...
                $sender = $message_thread_details->message_to_sender->name;
                ...
                <div class="m-body" id="message_body">
                  ...
                  >
                    <div class="user-name">
                      <h4 class="fz-16-sb-black">{{ $sender }}</h4>
                    </div>
                    <div
                      class="user-photo d-flex align-items-center g-10"
                    >
                      <div class="img">
                        <img src="{{ get_user_image($message_thread_details->sender) }}" alt="" />
                      </div>
                  ...
                  <div class="custom-height" id="scroll">
                    @include('customer.customer_messages.' .$message_inner_page_name)
                  </div>
                </div>
                @else
                  @include('customer.customer_messages.' .$message_inner_page_name)
                ...
                <form method="post" action="{{ route('customerReplyMessage', ['param1' => $current_message_thread_code]) }}" class="needs-validation" novalidate name="chat-form" id="chat-form">
                  @csrf
                  <div
                    class="message-send-option d-flex align-items-center flex-wrap g-20 pt-40"
                  >
                    <!-- Text Message -->
                    <div
                      class="message-input d-flex justify-content-start align-items-center"
                    >
                      <input
                        type="text"
  ...
  $(document).ready(function() {
    ...
    setInterval(function() {
      ...
      var message_thread_code = '<?= $current_message_thread_code ?>';
        let url = "{{ route('getSingleMassege', ['param1' => ":message_thread_code"]) }}";
