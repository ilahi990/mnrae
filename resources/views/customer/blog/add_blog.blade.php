        ...
        <div class="send-message-box">
            <div class="d-flex justify-content-between align-items-center flex-wrap g-12 mb-16">
                <div class="tableTitle-3">
                    <h4 class="fz-24-sb-black">{{ get_phrase('Add a new blog') }}</h4>
                </div>
                <!-- Button -->
                <a href="{{ route('blogList') }}" class="back-listing cg-10"><i class="fa-solid fa-arrow-left"></i> {{ get_phrase('Back to blog list') }}</a>
            </div>
            <form action="{{ route('blogAdd') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="mb-20">
                    <label for="title" class="form-label">{{ get_phrase('Title') }}*</label>
                    <input type="text" id="title" class="form-control" placeholder="Enter blog title" name="title" required>
                </div>
                <div class="mb-20">
                ...
                <div class="mb-20">
                    <label for="keywords" class="form-label">{{ get_phrase('Description') }}</label>
                    <textarea name="description" id="description" class="form-control" cols="30" rows="10" placeholder="Write something..."></textarea>
                </div>
                <div class="mb-21 blog-img-show">
                    <h5>{{ get_phrase('Blog thumbnail') }}</h5>
                    <div class="card">
                        <img  id="file-ip-1-preview" class="img-fluied" src="{{ asset('public/uploads/blog/placeholder.jpg') }}" alt="img" name="image" >
                         <div class="card-body">
                ...
                <div class="mb-20">
                    ...
                    <input type="text" id="meta_title" class="form-control" placeholder="Enter meta title" name="meta_title">
                </div>
                <div class="mb-20">
                    <label for="meta_keywords" class="form-label">{{ get_phrase('Meta keywords') }}</label>
                    <input type="text" id="meta_keywords" class="form-control" placeholder="Enter meta keywords" name="meta_keywords">
                </div>
                <div class="mb-20">
                    <label for="meta_description" class="form-label">{{ get_phrase('Meta Description') }}</label>
                    <textarea class="form-control" id="meta_description" name="meta_description" placeholder="Type your description"></textarea>
                </div>
                <div class="mb-20">
                    <label for="og_title" class="eForm-label">{{ get_phrase('Og Title') }}</label>
                   <input type="text" class="form-control" data-role="tagsinput" id = "og_title" name="og_title"/>
                </div>
                <div class="mb-20">
                    <label for="canonical " class="eForm-label">{{ get_phrase(' Canonical Url') }}</label>
                    <input type="text" class="form-control" data-role="tagsinput" id = "canonical " name="canonical" placeholder="https://example.com/dresses/cocktail?gclid=ABCD."/>
                </div>
                <div class="mb-20">
                    <label for="og_description" class="eForm-label">{{ get_phrase('Og Description') }}</label>
                    <Textarea class="form-control" id="og_description" name="og_description" type="text"></Textarea>
                </div>
                <div class="mb-20">
                ...
                <div class="mb-20">
                    ...
                    <Textarea class="form-control" id="json_ld" name="json_ld" placeholder='<script src="https://cdn.jsdelivr.net/npm/json ld"></script>'></Textarea>
                </div>
                <div class="mt-30">
