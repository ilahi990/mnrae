    ...
    public function update(Request $request)
    {
        ...
        if (array_key_exists('is_addon', $config) && strval($config['is_addon']) == "1") {
            if ($config['addon_version']['minimum_required_version'] == "0") {
                return redirect()->back()->with('message', get_phrase('Addon installed successfully'));
            } else {
                return redirect()->back()->with('message', get_phrase('Addon updated successfully'));
            }
        } else {
            DB::table('system_settings')->where('key', 'version')->update(['value' => $config['product_version']['update_version']]);
            return redirect()->back()->with('message', get_phrase('Version updated successfully'));
        }
    }
